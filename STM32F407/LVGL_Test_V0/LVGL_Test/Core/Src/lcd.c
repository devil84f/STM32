...
{ 	
  ...
  GPIO_InitStructure.Pin = GPIO_PIN_12;//PB1 推挽输出,控制背光
  GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_PP;//普通推挽输出模式
  GPIO_InitStructure.Pull = GPIO_PULLUP;//输出
  GPIO_InitStructure.Speed = GPIO_SPEED_HIGH;//100MHz
  HAL_GPIO_Init(GPIOD, &GPIO_InitStructure);//初始化 //PB15 推挽输出,控制背光
  HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_SET);  // 亮屏
  ...
  FSMC_NORSRAMInitStructure.Init.NSBank = FSMC_NORSRAM_BANK1;//  这里我们使用NE1 
  FSMC_NORSRAMInitStructure.Init.DataAddressMux = FSMC_DATA_ADDRESS_MUX_DISABLE; // 不复用数据地址
  FSMC_NORSRAMInitStructure.Init.MemoryType =FSMC_MEMORY_TYPE_SRAM;// FSMC_MemoryType_SRAM;  //SRAM   
  FSMC_NORSRAMInitStructure.Init.MemoryDataWidth = FSMC_NORSRAM_MEM_BUS_WIDTH_16;//存储器数据宽度为16bit   
  FSMC_NORSRAMInitStructure.Init.BurstAccessMode = FSMC_BURST_ACCESS_MODE_DISABLE;// FSMC_BurstAccessMode_Disable; 
  FSMC_NORSRAMInitStructure.Init.WaitSignalPolarity = FSMC_WAIT_SIGNAL_POLARITY_LOW;
	FSMC_NORSRAMInitStructure.Init.AsynchronousWait= FSMC_WRAP_MODE_DISABLE; 
  FSMC_NORSRAMInitStructure.Init.WrapMode = FSMC_WRAP_MODE_DISABLE;   
  FSMC_NORSRAMInitStructure.Init.WaitSignalActive = FSMC_WAIT_TIMING_BEFORE_WS;  
  FSMC_NORSRAMInitStructure.Init.WriteOperation = FSMC_WRITE_OPERATION_ENABLE;	//  存储器写使能
  FSMC_NORSRAMInitStructure.Init.WaitSignal = FSMC_WAIT_SIGNAL_DISABLE;   
  FSMC_NORSRAMInitStructure.Init.ExtendedMode = FSMC_EXTENDED_MODE_ENABLE; // 读写使用不同的时序

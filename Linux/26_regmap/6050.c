...
static int mpu6050_read_regs(struct mpu6050_dev *dev, u8 reg, void *val, int len)
{
    ...
    struct i2c_client *mpu6050_client = dev->private_data;
	...
	mpu6050_msg[1].buf = val;					//读取得到的数据保存位置
	mpu6050_msg[1].len = len;				//读取长度
...
static int mpu6050_init(struct mpu6050_dev *dev)
{
    ...
    mpu6050_write_reg(dev, MPU6050_PWR_MGMT_1, 0x01);
